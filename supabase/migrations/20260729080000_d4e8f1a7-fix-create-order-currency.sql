-- CRITICAL FIX: the server-side create_order() function — the actual,
-- authoritative order-creation logic, not just the client-side cart
-- display — summed each line item's `unit_price` directly with NO
-- currency conversion, then hardcoded the resulting order's `currency` to
-- 'USD' regardless of what currency the products were actually priced in.
-- For a product priced in any non-USD currency (or a cart mixing
-- currencies), this silently created a wrong `total_amount` on the actual
-- order record — the same bug fixed client-side in the cart/checkout UI
-- (Module 18), but here it's the number that actually gets charged and
-- stored, which the client-side fix alone could never correct.
--
-- Fix: a small `fx_rates` reference table (server-side, since a live HTTP
-- call inside a checkout transaction would be slow and adds an external
-- dependency to the critical path) that create_order() converts every line
-- item into USD through before summing. Kept fresh by a new cron route
-- (see the companion migration + src/routes/api/public/cron/sync-fx-rates.ts)
-- that mirrors the exact same rates the customer-facing CurrencyContext
-- already fetches live — same source, same numbers, just cached
-- server-side for checkout-time reliability.

CREATE TABLE IF NOT EXISTS public.fx_rates (
  currency_code text PRIMARY KEY,
  rate_per_usd numeric NOT NULL, -- units of this currency per 1 USD
...
DO $$ BEGIN
  CREATE POLICY "fx_rates writable by admin only"
    ON public.fx_rates FOR ALL
    USING (has_role(auth.uid(), 'admin'::app_role))
    WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Seed with the same values used as the client-side fallback
-- (src/context/CurrencyContext.tsx's defaultCurrencies) so server-side
-- conversion is sane even before the sync cron has run once.
INSERT INTO public.fx_rates (currency_code, rate_per_usd) VALUES
  ('USD', 1),
  ('AED', 3.6725),
  ('SAR', 3.75),
  ('BDT', 120.5),
...
-- Helper: USD-equivalent of `amount` denominated in `currency`. Unknown
-- currencies fail safe to a 1:1 rate (treated as USD) rather than raising
-- — an order should never hard-fail because a currency code was typo'd or
-- a new currency hasn't been added to fx_rates yet; it should just not
-- silently apply a wrong conversion multiplier either. 1:1 is the least
-- harmful assumption for a currency this table doesn't recognize.
...
AS $$
  SELECT amount / COALESCE(
    (SELECT rate_per_usd FROM public.fx_rates WHERE currency_code = COALESCE(currency, 'USD')),
    1
  );
$$;

-- Re-create create_order with currency-aware totals. Signature is
-- unchanged (drop-in replacement — every existing caller keeps working).
...
BEGIN
  ...
  LOOP
    ...
    SELECT id, title, price, currency, is_active, stock_quantity, gallery_urls
      INTO v_product
    ...
    IF (v_item->>'variant_id') IS NOT NULL AND length(v_item->>'variant_id') > 0 THEN
      ...
      -- Variant additional_price is assumed to be in the SAME currency as
      -- the parent product's price (it's a delta, not an independent
      -- price) — this matches how the client-side VariantSelector treats
      -- it (Module 18).
      v_unit_price := v_unit_price + COALESCE(v_variant.additional_price, 0);
    END IF;

    -- THE FIX: convert this line's unit price to USD BEFORE accumulating
    -- into v_subtotal. Previously this summed raw v_unit_price values
    -- regardless of each product's actual currency — meaningless
    -- arithmetic for any cart with a non-USD-priced product, or any mix.
    v_unit_price_usd := public.to_usd(v_unit_price, v_product.currency);
    v_subtotal := v_subtotal + (v_unit_price_usd * v_qty);
  END LOOP;
  ...
  IF p_coupon_code IS NOT NULL AND length(trim(p_coupon_code)) > 0 THEN
    ...
    ELSE
      -- Fixed-amount coupons are assumed to be denominated in USD, matching
      -- v_subtotal now always being USD-pivot. The `coupons` table has no
      -- currency column (a pre-existing gap — see
      -- docs/architecture/OUTSTANDING_ITEMS_REPORT.md item #1) — if your
      -- existing fixed coupons were created assuming BDT amounts, they will
      -- now discount the wrong amount. Verify your coupon data.
  ...
  LOOP
    ...
    SELECT id, title, price, currency, gallery_urls INTO v_product
    ...
    IF (v_item->>'variant_id') IS NOT NULL AND length(v_item->>'variant_id') > 0 THEN
      ...
      IF v_variant.id IS NOT NULL THEN
        v_has_variant := true;
        v_variant_value := v_variant.value;
        v_unit_price := v_unit_price + COALESCE(v_variant.additional_price, 0);
        v_title := v_title || ' (' || COALESCE(v_variant.value, '') || ')';
    ...
    -- order_items.unit_price is stored in USD (matching orders.currency =
    -- 'USD') so the order-receipt display (Module 19,
    -- formatInCurrency(item.unit_price, order.currency)) is internally
    -- consistent — previously this stored the raw native-currency price
    -- while the order claimed to be in USD, an inconsistency between the
    -- order total and its own line items.
    INSERT INTO public.order_items (
      ...
      v_title, v_qty, public.to_usd(v_unit_price, v_product.currency), v_image, v_variant_value
    );
  END LOOP;
