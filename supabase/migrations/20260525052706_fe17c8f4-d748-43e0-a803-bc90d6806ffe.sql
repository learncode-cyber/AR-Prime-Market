...
BEGIN
  ...
  IF v_user IS NULL THEN
    -- Use UUIDs (built-in, no extension needed) to build a 48-char hex token
...
-- Also fix generate_order_number trigger which uses gen_random_bytes
...
AS $function$
BEGIN
    NEW.order_number := 'ORD-' || to_char(now(), 'YYYYMMDD') || '-' || upper(substring(replace(gen_random_uuid()::text, '-', ''), 1, 10));
    RETURN NEW;
END; $function$;